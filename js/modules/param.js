...
const Param = (function () {
  ...
  function setChartProps({ index, properties }) {
    ...
    //transformProperties(newValues, "set"); //unFlatten ? _unFlatten(newValues) : newValues;
...
function transformProperties(obj, action) {
  // keys starting with one underescore have values as json
  // keys starting with more that one underscrore are to be ignored
  const validPrefixes = ["plan_"]; //to do remove later
  const seperator = "_";
  ...
  for (const key in obj) {
    if (action === "get" && isValidPrefix(key)) {
      Object.assign(transformedObj, parse(key));
      continue;
    }
    if (action === "set") {
      const [prefix, suffix] = key.split(seperator);
      const newKey = prefix + seperator;
      if (validPrefixes.includes(newKey) && suffix) {
        if (!prefixexDone.includes[prefix])
          transformedObj[newKey] = stringify(key);
        continue;
      }
    }
