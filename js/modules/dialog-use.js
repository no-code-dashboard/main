...
// }
//////////////////////////////////////////////////////////////////// config dialog
function updateInitialValues(elements, initialvalues) {
  if (!initialvalues) return elements;
  return elements.map((e) => {
    if (!e.name) return e;
    const value = initialvalues[e.name];
    if (!value) return e;
    return { ...e, value };
  });
}
...
function showCalloutMenu(key, reCreateCharts) {
  const buttons = [
  ...
  function callback({ type, target }) {
    ...
    Dialog.close();
    const label = target.textContent;
    if (label === buttons[0].label) showCalloutConfigDialog(key);
    if (label === buttons[1].label) removeCallout(key);
  }
...
function showLoadFileDialog({ header, extention, loadFunction }) {
  ...
  async function callback({ target, type }) {
    console.log({ target, type, data: Dialog.data() });
...
//   //   "https://www.drikpanchang.com/bengali/bengali-month-panjika.html?geoname-id=1275004&date=" +
//   //     date
//   // );

//   if (true) {
//     // Use a listener for a more modern approach
//     newWindow.addEventListener("DOMContentLoaded", () => {
//       const [bongDM, paksha, bongEra] = document
//         .querySelector(".dpPHeaderLeftContent")
//         .innerText.split("\n"); //.replaceAll("\n"," ")
//       const [bongD, bongMoth] = bongDM.split(" ");
//       const bongDate = bongD.replace(",", "");
//       const bongYear = bongEra.split(" ")[0];
//       const bong = `${bongDate} ${bongMoth} ${bongYear} `;
//       const greg = document
//         .querySelector(".dpPHeaderRightContent")
//         .innerText.replaceAll("\n", " ");
//       // .split(" ");
//       const banner = bong + " - " + greg;
//       const bigDates = Array.from(document.querySelectorAll(".dpBigDate"));
//       const datePair = bigDates.map((span) => {
//         const bDate = span.nextElementSibling.innerText;
//         const gDateDay = span.textContent;
//         const gDate = gDateDay.substring(0, gDateDay.length === 5 ? 2 : 1);
//         const gDay = gDateDay.replace(gDate, "");
//         const tab = "\t";
//         return [
//           gDay,
//           gDate,
//           bDate,
//           Number(bongDate) === Number(bDate) ? banner : "",
//         ].join(tab);
//       });
//       return (datePair.join("\n"));
//     });
//   }
// })("01/01/1959");
