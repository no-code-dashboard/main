...
const Counter = (function () {
  ...
  async function getCountsFromFile(inputJSON) {
    ...
    const maxRecorsToRead = config
      ? config["max-recrds-to=read"] ?? 2000
      : 2000;
    if (!config) {
    ...
    function includeRowInChart(key, row) {
      ...
      function init() {
        ...
        if (!allCounts.memo[key]) allCounts.memo[key] = {};
        // console.log(output)
        //action:"include", where: [{columnName: 'STATUS', op: 'eq', operand: Array(2)}]
      ...
      function filterAction() {
        ...
        // console.log(allCounts.memo[key].chartFilter)
        if (!action) return true;
        let whereConditionMet = true;
        const results = where.map((condition) => ({
          met: getConditionResult(condition),
          join: condition.join,
        }));
        for (let i = 0; i < results.length; i++) {
          const { met, join } = results[i];
          if (typeof join !== "boolean") break;
          whereConditionMet = join
            ? whereConditionMet && met
            : whereConditionMet || met;
        ...
        //   }
        // }
        return action == "include" ? whereConditionMet : !whereConditionMet;
